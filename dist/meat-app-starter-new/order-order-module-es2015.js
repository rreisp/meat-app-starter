...
let DeliveryCostsComponent = class DeliveryCostsComponent {
    constructor() { }
    ngOnInit() {
    }
    total() {
...
let OrderItemsComponent = class OrderItemsComponent {
    ...
    emitIncreaseQty(item) {
        this.increaseQty.emit(item);
    }
    emitDecreaseQty(item) {
...
let OrderComponent = OrderComponent_1 = class OrderComponent {
    constructor(orderService, router, formBuilder) {
    ...
    checkOrder(order) {
        order.orderItems = this.cartItems()
            .map((item) => new _order_model__WEBPACK_IMPORTED_MODULE_3__["OrderItem"](item.quantity, item.menuItem.id));
...
class OrderItem {
    constructor(quantity, menuId) {
        this.quantity = quantity;
        this.menuId = menuId;
    }
}
OrderItem.ctorParameters = () => [
    { type: Number },
    { type: String }
];
